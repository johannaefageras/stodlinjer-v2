import { getCollection } from "astro:content";
import categoriesData from "../../content/categories.json";

// Shared sample + opening-hours formatting for the throwaway card mockups.
// Delete alongside src/components/mockups/ and the mockup pages once a card
// direction is chosen.

const categories = categoriesData;
const catById = Object.fromEntries(categories.map((c) => [c.id, c]));

const DAY_ORDER = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"] as const;
const DAY_LABEL: Record<string, string> = {
  mon: "Mån",
  tue: "Tis",
  wed: "Ons",
  thu: "Tor",
  fri: "Fre",
  sat: "Lör",
  sun: "Sön",
};
type Hours = { days: string[]; open: string; close: string };

const fmtTime = (t: string) =>
  t === "24:00" ? "24" : t.endsWith(":00") ? t.slice(0, 2) : t;
const fmtRange = (open: string, close: string) =>
  open === "00:00" && close === "24:00"
    ? "Dygnet runt"
    : `${fmtTime(open)}–${fmtTime(close)}`;

function groupDays(days: string[]): string {
  const idx = days
    .map((d) => DAY_ORDER.indexOf(d as (typeof DAY_ORDER)[number]))
    .filter((i) => i >= 0)
    .sort((a, b) => a - b);
  if (!idx.length) return "";
  const runs: [number, number][] = [];
  let start = idx[0];
  let prev = idx[0];
  for (let k = 1; k < idx.length; k++) {
    if (idx[k] === prev + 1) {
      prev = idx[k];
      continue;
    }
    runs.push([start, prev]);
    start = idx[k];
    prev = idx[k];
  }
  runs.push([start, prev]);
  return runs
    .map(([a, b]) =>
      a === b
        ? DAY_LABEL[DAY_ORDER[a]]
        : `${DAY_LABEL[DAY_ORDER[a]]}–${DAY_LABEL[DAY_ORDER[b]]}`,
    )
    .join(", ");
}

const is247 = (entries: Hours[]) => {
  const all = new Set<string>();
  let allFull = true;
  for (const e of entries) {
    for (const d of e.days) all.add(d);
    if (!(e.open === "00:00" && e.close === "24:00")) allFull = false;
  }
  return allFull && all.size === 7;
};

const detailHours = (entries: Hours[]) =>
  entries.map((e) => ({ dayRange: groupDays(e.days), time: fmtRange(e.open, e.close) }));

function shortHours(entries: Hours[]): string | null {
  if (!entries.length) return null;
  if (is247(entries)) return "Dygnet runt";
  const rows = detailHours(entries);
  const shown = rows
    .slice(0, 2)
    .map((r) => `${r.dayRange} ${r.time}`)
    .join(" · ");
  return rows.length > 2 ? `${shown} …` : shown;
}

function closedDays(entries: Hours[]): string | null {
  const covered = new Set<string>();
  for (const e of entries) for (const d of e.days) covered.add(d);
  const off = DAY_ORDER.filter((d) => !covered.has(d));
  return off.length ? groupDays(off as unknown as string[]) : null;
}

// Live "open now" status for the card chip, evaluated in Europe/Stockholm.
export type LiveStatus = { tone: "open" | "soon" | "closed"; label: string };

const toMin = (t: string) => {
  const [h, m] = t.split(":").map(Number);
  return h * 60 + m;
};
const clock = (m: number) =>
  `${String(Math.floor(m / 60)).padStart(2, "0")}:${String(m % 60).padStart(2, "0")}`;

function liveStatus(entries: Hours[]): LiveStatus | null {
  if (!entries.length) return null;
  if (is247(entries)) return { tone: "open", label: "Dygnet runt" };

  const parts = new Intl.DateTimeFormat("en-US", {
    timeZone: "Europe/Stockholm",
    weekday: "short",
    hour: "2-digit",
    minute: "2-digit",
    hour12: false,
  }).formatToParts(new Date());
  const get = (t: string) => parts.find((p) => p.type === t)?.value ?? "";
  const wdToIdx: Record<string, number> = {
    Mon: 0, Tue: 1, Wed: 2, Thu: 3, Fri: 4, Sat: 5, Sun: 6,
  };
  const todayIdx = wdToIdx[get("weekday")] ?? 0;
  const nowM = ((parseInt(get("hour"), 10) % 24) * 60) + parseInt(get("minute"), 10);

  // Flatten opening hours into per-weekday intervals (sun close 24:00 → 1440).
  const ivals = entries.flatMap((e) =>
    e.days
      .map((d) => DAY_ORDER.indexOf(d as (typeof DAY_ORDER)[number]))
      .filter((day) => day >= 0)
      .map((day) => ({ day, open: toMin(e.open), close: toMin(e.close) })),
  );

  // Open right now?
  if (ivals.some((iv) => iv.day === todayIdx && nowM >= iv.open && nowM < iv.close)) {
    return { tone: "open", label: "Öppet nu" };
  }

  // Otherwise the next opening within the coming week.
  for (let ahead = 0; ahead < 7; ahead++) {
    const day = (todayIdx + ahead) % 7;
    const next = ivals
      .filter((iv) => iv.day === day && (ahead > 0 || iv.open > nowM))
      .sort((a, b) => a.open - b.open)[0];
    if (!next) continue;
    if (ahead === 0) return { tone: "soon", label: `Öppnar ${clock(next.open)}` };
    const dayName = DAY_LABEL[DAY_ORDER[day]].toLowerCase();
    return { tone: "closed", label: `Öppnar ${dayName} ${clock(next.open)}` };
  }
  return { tone: "closed", label: "Stängt" };
}

export async function getMockupCards() {
  const lines = (
    await getCollection("supportLines", (e) => e.data.status === "active")
  ).sort((a, b) => {
    const f = Number(b.data.display.featured) - Number(a.data.display.featured);
    if (f !== 0) return f;
    const p = b.data.display.priority - a.data.display.priority;
    if (p !== 0) return p;
    return a.data.name.localeCompare(b.data.name, "sv");
  });

  type Line = (typeof lines)[number];
  const phoneWithHours = (l: Line) =>
    l.data.contactMethods.find((c) => c.channel === "phone" && c.openingHours.length) ??
    l.data.contactMethods.find((c) => c.openingHours.length);

  const noPhone = lines.find(
    (l) => !l.data.contactMethods.some((c) => c.channel === "phone" && c.value),
  );
  const hasClosedDays = (l: Line) => {
    const cm = phoneWithHours(l);
    if (!cm) return false;
    const covered = new Set<string>();
    for (const e of cm.openingHours) for (const d of e.days) covered.add(d);
    return covered.size > 0 && covered.size < 7;
  };
  const weekday =
    lines.find(hasClosedDays) ??
    lines.find((l) => {
      const cm = phoneWithHours(l);
      return cm && !is247(cm.openingHours as Hours[]);
    });

  const picks: Line[] = [];
  const add = (l?: Line) => {
    if (l && !picks.includes(l)) picks.push(l);
  };
  add(lines.find((l) => l.data.slug === "bris"));
  add(weekday);
  add(noPhone);
  add(lines.find((l) => l.data.slug === "diskrimineringsombudsmannen"));
  add(lines.find((l) => l.data.slug === "giftinformationscentralen"));
  for (const l of lines) {
    if (picks.length >= 9) break;
    add(l);
  }

  return picks.map((line) => {
    const cat = catById[line.data.category.id];
    const phoneCM = line.data.contactMethods.find(
      (c) => c.channel === "phone" && c.value,
    );
    const phone = phoneCM?.value ?? null;
    const telHref = phone ? `tel:${phone.replace(/\D/g, "")}` : null;
    const hoursCM = phoneWithHours(line);
    const entries = (hoursCM?.openingHours ?? []) as Hours[];
    return {
      name: line.data.name,
      shortDescription: line.data.shortDescription,
      categoryColor: cat.color,
      categoryIcon: cat.icon,
      categoryId: cat.id,
      categoryLabel: cat.label,
      phone,
      telHref,
      primaryLabel: line.data.display.primaryLabel,
      hoursShort: shortHours(entries),
      hoursDetail: detailHours(entries),
      hoursClosed: closedDays(entries),
      status: liveStatus(entries),
      href: `/stodlinjer/${line.data.slug}/`,
    };
  });
}
